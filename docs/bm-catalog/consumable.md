...
- **순수 시간축 콘텐츠 게이트 (웹툰 "기다리면 무료"류) — 소모성 재화와는 다른 계열**: 카카오페이지의 "기다리면 무료"(기다무)는 일정 시간이 지나면 다음 회차가 무료로 열리고, 결제하면 그 대기 시간을 건너뛸 수 있는 구조다. 스태미나·목숨처럼 "반복 사용 가능한 재화"를 소모·충전하는 것이 아니라, 콘텐츠 한 단위(챕터/회차)마다 한 번만 적용되는 시간 잠금이라는 점에서 이 문서가 다루는 소모성 재화와는 결이 다르다. 게임에서는 [SLG 장르](../genres/slg.md)의 건설/훈련 대기시간이 이 방식에 가장 가깝다. 유료 결제가 "재화 구매"가 아니라 "순수하게 기다림을 사는 것"이라는 프레이밍이 소모성 재화 대비 가격 저항감을 낮춘다는 점이 이 모델의 핵심 심리 기제로 꼽힌다. *(타산업 사례이며 국내 게임에서의 직접 적용 사례는 검증 필요)*
